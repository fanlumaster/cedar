...
// 一种特别的让编译器给出具有特定信息的报错
#define STATIC_ASSERT(e, msg) typedef char msg[(e) ? 1 : -1]

namespace cedar {
// typedefs
typedef unsigned char uchar; // 8 位无符号整数
// 两个 NaN
...
public:
  // TODO: 什么叫 no_value, 什么叫 no_path，什么叫 value_limit
  enum error_code { CEDAR_NO_VALUE = NO_VALUE, CEDAR_NO_PATH = NO_PATH, CEDAR_VALUE_LIMIT = 2147483647 };
  typedef value_type result_type; // TODO: result 是什么
  ...
  struct node {
    union {
      int base_;
      value_type value; // TODO: 这里为什么要多一个 value_type
    }; // negative means prev empty index
    int check;                                                                         // negative means next empty index
    node(const int base__ = 0, const int check_ = 0) : base_(base__), check(check_) {} // 默认都为 0
  ...
  // interfance
  template <typename T> T exactMatchSearch(const char *key) const { return exactMatchSearch<T>(key, std::strlen(key)); }
  template <typename T> T exactMatchSearch(const char *key, size_t len, size_t from = 0) const {
    union {
      int i;
      value_type x;
    } b;
  ...
  template <typename T> size_t commonPrefixSearch(const char *key, T *result, size_t result_len) const { return commonPrefixSearch(key, result, result_len, std::strlen(key)); }
  template <typename T> size_t commonPrefixSearch(const char *key, T *result, size_t result_len, size_t len, size_t from = 0) const {
    size_t num = 0;
  ...
  template <typename T> size_t commonPrefixPredict(const char *key, T *result, size_t result_len) { return commonPrefixPredict(key, result, result_len, std::strlen(key)); }
  template <typename T> size_t commonPrefixPredict(const char *key, T *result, size_t result_len, size_t len, size_t from = 0) {
  ...
  value_type traverse(const char *key, size_t &from, size_t &pos) const { return traverse(key, from, pos, std::strlen(key)); }
  ...
  template <typename T> value_type &update(const char *key, size_t &from, size_t &pos, size_t len, value_type val, T &cf) {
    if (!len && !from)
...
// 快速加载，那么，就一次性全部分配
    ...
    if (!_array || !_ninfo || !_block) // 空间分配失败
#else
    if (!_array)
#endif
      _err(__FILE__, __LINE__, "memory allocation failed\n");
    if (size_ != std::fread(_array, sizeof(node), size_,
                            fp)) // 读取数据，填充到 _array 中的所有 node 中
      return -1;
    std::fclose(fp);
    ...
    if (size_ != std::fread(_ninfo, sizeof(ninfo), size_, fp) || size_ != std::fread(_block, sizeof(block), size_ >> 8, fp) << 8)
      return -1;
    std::fclose(fp);
    _capacity = _size;
#endif
...
private:
  ...
  template <typename T> static void _realloc_array(T *&p, const int size_n, const int size_p = 0) {
  ...
  void _restore_ninfo() {
    ...
    for (int to = 0; to < _size; ++to) {
      ...
      if (const uchar label = static_cast<uchar>(base ^ to)) // skip leaf
        _push_sibling(static_cast<size_t>(from), base, label, !from || _ninfo[from].child || _array[base ^ 0].check == from);
    }
  }
  void _restore_block() {
  ...
  bool _consult(const int base_n, const int base_p, uchar c_n, uchar c_p) const {
    do
      if (!(c_p = _ninfo[base_p ^ c_p].sibling))
        return false;
  ...
  template <typename T> int _resolve(size_t &from_n, const int base_n, const uchar label_n, T &cf) {
    ...
    uchar *const last = flag ? _set_child(first, base_n, _ninfo[from_n].child, label_n) : _set_child(first, base_p, _ninfo[from_p].child);
