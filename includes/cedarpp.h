...
#pragma GCC diagnostic ignored "-Wunused-local-typedefs"
    STATIC_ASSERT(sizeof(value_type) <= sizeof(int), value_type_is_not_supported___maintain_a_value_array_by_yourself_and_store_its_index_to_trie);
#pragma GCC diagnostic warning "-Wunused-local-typedefs"
  ...
  value_type traverse(const char *key, npos_t &from, size_t &pos) const { return traverse(key, from, pos, std::strlen(key)); }
  ...
  template <typename T> value_type &update(const char *key, npos_t &from, size_t &pos, size_t len, value_type val, T &cf) {
    if (!len && !from)
    ...
    if (!offset) { // node on trie
      for (const uchar *const key_ = reinterpret_cast<const uchar *>(key); _array[from].base >= 0; ++pos) {
    ...
    if (offset >= sizeof(int)) { // go to _tail
      ...
      if (pos == len || tail[pos] == '\0') {
        ...
        if (pos == len)
          return _array[to].value += val; // set value on trie
  ...
  int open(const char *fn, const char *mode = "rb", const size_t offset = 0, size_t size_ = 0) {
    ...
    if (std::fseek(fp, static_cast<long>(offset), SEEK_SET) != 0)
      return -1;
    if (length_ != std::fread(_tail, sizeof(char), length_, fp) || size_ != std::fread(_array, sizeof(node), size_, fp))
      return -1;
    std::fclose(fp);
    ...
    *_length0 = 0;
    ...
    if (size_ != std::fread(_ninfo, sizeof(ninfo), size_, fp) || size_ >> 8 != std::fread(_block, sizeof(block), size_ >> 8, fp))
      return -1;
    std::fclose(fp);
    _capacity = _size;
    _quota = *_length;
...
private:
...
#endif
  void _set_result(result_type *x, value_type r, size_t = 0, npos_t = 0) const { *x = r; }
  ...
  template <typename T> int _resolve(npos_t &from_n, const int base_n, const uchar label_n, T &cf) {
